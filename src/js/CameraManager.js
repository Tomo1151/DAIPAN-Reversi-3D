...
export default class CameraManager {
  ...
  moveTo(x, y, z, lookAt, controlable, callback, step) {
    this.#gameManager.logger.log(
      ` |~ camera: move to (${x.toPrecision(3)}, ${y.toPrecision(3)}, ${z.toPrecision(3)})`,
    );
  ...
  move() {
    ...
    if (Math.abs(dx) < 0.05 && Math.abs(dy) < 0.05 && Math.abs(dz) < 0.05) {
      ...
      this.#gameManager.logger.log(
        ` |~ camera: moved to (${this.#target.x.toPrecision(3)}, ${this.#target.y.toPrecision(3)}, ${this.#target.z.toPrecision(3)})`,
      );
  ...
  set controlable(bool) {
    this.#rendererManager.controls.enabled = bool;
  }
  get position() {
