...
export default class GameSection extends Section {
  ...
  constructor(gameManager, rendererManager, cameraManager, scene) {
    ...
    const move = (e) => {
      ...
      this.rendererManager.raycaster.setFromCamera(
        this.rendererManager.mouse,
        this.rendererManager.camera,
      );

      switch (this.#mode) {
        case GameSection.MODE_PUT:
          let intersects = this.rendererManager.raycaster.intersectObjects(
            this.#hitboxes,
          );
        ...
        case GameSection.MODE_BANG:
          ...
          let intersect = this.rendererManager.raycaster.intersectObject(
            this.#base,
          )[0];
          if (intersect) {
            this.#onBase = true;
  ...
  async animationFlip(disk_num, order) {
    ...
    let action = this.#animationMixers[disk_num].clipAction(
      disk.animations[order],
    );
    ...
    this.logger.log(
      ` |... animation start [flip :to${order == Disk.BLACK ? "B" : "W"}]`,
    );
  ...
  async animationPut(disk_num, order) {
    ...
    this.logger.log(
      ` |... animation start [put :${order == Disk.BLACK ? "B" : "W"}]`,
    );
